...
const getShoes = () => {
  shoesList.innerHTML = ''
  fetch(url + `${filterPrice.length ? '_sort=price&_order=' + filterPrice + '&' : ''}${filterCategory.length ? 'category=' + filterCategory + '&' : ''}${filterGender.length ? 'gender=' + filterGender + '&' : '' }${filterBrands.length ? 'brand=' + filterBrands : ''}`)
